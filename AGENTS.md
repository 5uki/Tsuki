...
- **Playwright MCP 测试为必备项** — 任何界面/交互改动必须用 Playwright MCP 验证并在回复中说明已验证的要点
- **任务性质确认** — 确认任务是否需要改动代码，如果是计划或技术文档不要动源代码
- **需求优先 + 伴随式重构** — 以满足用户需求为主；允许在实现需求过程中做“小步、可验证、范围最小”的重构来降低复杂度/重复，但禁止为了重构而重构（不得拖慢交付）。
- **计划内大重构（允许，但得带脑子）** — 项目早期可做结构级重构/重排目录/清理依赖，但必须同时满足：
  - 明确范围与目标（最多 1..2 个主目标）
  - 分阶段交付（每阶段可运行/可验证/可回滚），禁止“推倒重来然后祈祷”
  - 以 `REQUIREMENTS.md` 为验收底线：功能不回退、约束不被打破
- **兼容性策略** — 默认只支持现代环境（Chromium 最新/近期版本 + Node.js LTS）；不做老旧浏览器/非主流环境兼容，不引入 polyfill/降级分支；除非需求明确提出且验收标准写清。

# CSS 约束
- CSS 允许很强，但也最容易把项目写成“屎山喷泉”（尤其是早期没规矩的时候）。
- **必须 Token 化**：颜色/间距/圆角/阴影/字号等一律走 Design Tokens（CSS Variables），禁止在组件里硬编码颜色值；主题切换相关遵循 `REQUIREMENTS.md` 的 `--tsuki-` 约束。
- **禁止样式到处散**：避免行内 style、避免随手写一堆 “临时 class”；优先组件级样式隔离（CSS Modules/Scoped）+ 少量全局 reset。
- **禁止为兼容而污染代码**：不写兼容分支、不写老浏览器 hack、不引 polyfill（除非需求明确写了验收）。
...
## 1) 编码与文本安全
- 所有源码文件统一 UTF-8 编码。
- 禁止“整文件重写式”改中文文案；优先做最小范围 patch，避免引入乱码。
- 涉及中文文案修改时，修改后必须二次检索乱码字符（如 `�`、异常 mojibake）。
...
- 主题按“三色主 token”维护：
  ...
  - 次要色（subtle）
- 黑/白等中性色不计入主题色，可独立定义。
- 字体颜色规则：默认黑色；强调态使用强调色；避免同区域混用多套文字色 token。

### 3) 优先工具调用而非Powershell/Bash
...
## 本地站点约定
- 默认本地开发站点已常驻运行；除非用户明确要求，否则不要重复启动 dev server。
